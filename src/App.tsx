...
function App() {
  ...
  useEffect(() => {
    ...
    .fromTo('.hero-headline span',
      ...
      { y: 0, opacity: 1, rotateX: 0, duration: 1, stagger: 0.04, ease: 'power3.out' },
      '-=0.8'
    )
    .fromTo('.hero-sub',
      { y: 30, opacity: 0 },
    ...
    sections.forEach(({ ref, class: className }) => {
      ...
      gsap.fromTo(`.${className}-headline`,
        { opacity: 0, x: className === 'investors' || className === 'mentorship' || className === 'howitworks' ? -60 : 60 },
        {
          opacity: 1,
          x: 0,
          duration: 0.9,
  ...
  return (
    <>
      ...
      <div className="relative min-h-screen bg-[#0B0F17]">
        ...
        {/* Navigation */}
        <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${navVisible ? 'bg-[#0B0F17]/90 backdrop-blur-md py-4' : 'py-6'}`}>
          <div className="px-6 lg:px-12 flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center gap-8">
              <button onClick={() => scrollToSection(foundersRef)} className="nav-link">For Founders</button>
              <button onClick={() => scrollToSection(investorsRef)} className="nav-link">For Investors</button>
              <button onClick={() => scrollToSection(featuredRef)} className="nav-link">Startups</button>
              <button onClick={() => scrollToSection(contactRef)} className="nav-link">Contact</button>
              <button onClick={() => scrollToSection(contactRef)} className="btn-primary text-sm group">
                Apply to Pitch
          ...
          {/* Mobile menu */}
          <div className={`lg:hidden absolute top-full left-0 right-0 bg-[#0B0F17]/98 backdrop-blur-md border-t border-white/10 transition-all duration-300 overflow-hidden ${mobileMenuOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="py-6 px-6 space-y-4">
              <button onClick={() => scrollToSection(foundersRef)} className="block w-full text-left text-white/80 py-3 hover:text-white hover:bg-white/5 rounded-lg px-4 transition-all">For Founders</button>
              <button onClick={() => scrollToSection(investorsRef)} className="block w-full text-left text-white/80 py-3 hover:text-white hover:bg-white/5 rounded-lg px-4 transition-all">For Investors</button>
              <button onClick={() => scrollToSection(featuredRef)} className="block w-full text-left text-white/80 py-3 hover:text-white hover:bg-white/5 rounded-lg px-4 transition-all">Startups</button>
              <button onClick={() => scrollToSection(contactRef)} className="block w-full text-left text-white/80 py-3 hover:text-white hover:bg-white/5 rounded-lg px-4 transition-all">Contact</button>
              <button onClick={() => scrollToSection(contactRef)} className="btn-primary text-sm w-full justify-center mt-4">
                Apply to Pitch
              </button>
            </div>
          </div>
        ...
        {/* Section 1: Hero */}
        <section ref={heroRef} className="section-pinned flex items-center justify-center min-h-screen">
          <div className="hero-card card-stage w-[92vw] lg:w-[88vw] h-[70vh] lg:h-[64vh] mt-16 relative overflow-hidden group">
            ...
            <div className="absolute bottom-8 right-16 w-12 h-12 border border-[#2B7FFF]/20 rounded-full animate-pulse-slow" style={{ animationDelay: '1s' }} />
            ...
            <div className="relative z-10 h-full flex flex-col justify-center px-8 lg:px-16">
              ...
              <h1 className="hero-headline text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold text-white max-w-3xl leading-[0.95] mb-6" style={{ perspective: '1000px' }}>
                {'PITCH YOUR STARTUP TO INDIA\'S TOP INVESTORS'.split(' ').map((word, i) => (
                  <span key={i} className="inline-block mr-[0.25em]" style={{ transformStyle: 'preserve-3d' }}>{word}</span>
                ))}
              </h1>
              ...
              <div className="hero-cta flex flex-wrap gap-4">
                <button onClick={() => scrollToSection(contactRef)} className="btn-primary group">
                  Apply to Pitch
                  <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </button>
                <button onClick={() => scrollToSection(featuredRef)} className="btn-secondary group">
        ...
        <section ref={foundersRef} className="section-pinned flex items-center justify-center min-h-screen">
        ...
        <section ref={investorsRef} className="section-pinned flex items-center justify-center min-h-screen">
        ...
        <section ref={fundingRef} className="section-pinned flex items-center justify-center min-h-screen">
          <div className="funding-card card-stage w-[92vw] lg:w-[88vw] h-[70vh] lg:h-[64vh] relative overflow-hidden group">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center items-end text-right px-8 lg:px-16">
              ...
              <p className="funding-body text-lg text-white/80 max-w-lg">
                Get in front of angels, VCs, and media. Turn one pitch into a month of momentum.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={mentorshipRef} className="section-pinned flex items-center justify-center min-h-screen">
          <div className="mentorship-card card-stage w-[92vw] lg:w-[88vw] h-[70vh] lg:h-[64vh] relative overflow-hidden group">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center px-8 lg:px-16">
              ...
              <p className="mentorship-body text-lg text-white/80 max-w-lg">
                Weekly office hours. GTM playbooks. Investor intros that actually reply.
              </p>
            </div>
          </div>
        </section>
        ...
        {/* Section 6: Community */}
        <section ref={communityRef} className="section-pinned flex items-center justify-center min-h-screen">
          <div className="community-card card-stage w-[92vw] lg:w-[88vw] h-[70vh] lg:h-[64vh] relative overflow-hidden group">
            ...
            <div className="relative z-10 h-full flex flex-col justify-center items-end text-right px-8 lg:px-16">
              ...
              <p className="community-body text-lg text-white/80 max-w-lg">
                Thousands of founders. Hundreds of pitches. One network that helps you scale.
              </p>
            </div>
          </div>
        </section>
        ...
        <section ref={howItWorksRef} className="section-pinned flex items-center justify-center min-h-screen">
            ...
            <div className="relative z-10 h-full flex flex-col lg:flex-row justify-between px-8 lg:px-16 py-12">
              <div className="flex flex-col justify-center max-w-xl">
                ...
                <p className="howitworks-body text-lg text-white/80">
                  Submit your deck. If selected, pitch live. Get feedback and term sheets.
                </p>
              </div>
                ...
                <div className="space-y-6">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-4 group cursor-pointer">
                      ...
                      <span className="text-white text-lg font-medium group-hover:text-[#FFD400] transition-colors">{step}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <section ref={featuredRef} className="py-24 lg:py-32 px-6 lg:px-12">
          ...
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredStartups.map(startup => (
              <div key={startup.id} className="startup-card-item startup-card group">
                ...
                <div className="p-6">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xl font-bold text-white group-hover:text-[#FFD400] transition-colors">{startup.name}</h3>
                    <span className="text-xs font-mono-label text-[#FFD400]">{startup.category}</span>
                  </div>
                  <p className="text-white/70 text-sm mb-4">{startup.description}</p>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-16">
              ...
              <p className="text-lg text-white/70 max-w-xl mx-auto">
                Connect with leading VCs, angels, and institutional investors from across India.
              </p>
            </div>
            
            <div className="investors-grid grid grid-cols-2 md:grid-cols-4 gap-6">
              {investorLogos.map((investor, i) => (
                <div key={i} className="investor-card bg-white/5 border border-white/10 rounded-2xl p-8 flex flex-col items-center justify-center hover:border-[#FFD400]/50 hover:bg-white/10 hover:scale-105 transition-all cursor-pointer group">
                  ...
                  <span className="text-white font-medium text-center">{investor.name}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              <div className="about-text">
                ...
                <div className="space-y-4 text-white/70 text-lg">
                  <p>
                    PitchConnect is a startup media platform that has shared thousands of founder stories.
                  </p>
                  <p>
                    We run live pitch events, publish operator playbooks, and connect builders to capital.
                  </p>
                  <p>
                    From metros to emerging hubs—if you're building, we want to hear from you.
                  </p>
                </div>
        ...
        <section className="py-24 lg:py-32 px-6 lg:px-12">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="apply-grid grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
              {[
                { icon: <TrendingUp className="w-8 h-8" />, label: 'D2C Brands' },
                ...
                { icon: <Briefcase className="w-8 h-8" />, label: 'Manufacturing' },
                { icon: <Globe className="w-8 h-8" />, label: 'Bharat-Focused' }
              ].map((item, i) => (
                <div key={i} className="apply-card bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center text-center hover:border-[#FFD400]/50 hover:bg-white/10 hover:scale-105 transition-all cursor-pointer group">
        ...
        {/* Section 12: Contact */}
        <section ref={contactRef} className="py-24 lg:py-32 px-6 lg:px-12 bg-[#F6F7FB]">
          <div className="max-w-7xl mx-auto">
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4 group cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="text-[#0B0F17] font-medium">hello@pitchconnect.in</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 group cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="text-[#0B0F17] font-medium">+91 80 4601 8842</div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 group cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="text-[#0B0F17] font-medium">Bangalore • Mumbai • Delhi NCR</div>
                    </div>
                  </div>
                </div>
              </div>
              
              <div className="contact-form bg-white rounded-3xl p-8 shadow-xl" style={{ perspective: '1000px' }}>
                <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-[#0B0F17] mb-2">Name</label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="block text-sm font-medium text-[#0B0F17] mb-2">Email</label>
                      <input 
                        type="email" 
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#2B7FFF] focus:ring-2 focus:ring-[#2B7FFF]/20 outline-none transition-all hover:border-gray-300"
                  ...
                  <div className="grid sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-[#0B0F17] mb-2">Startup Name</label>
                      <input 
                        type="text" 
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#2B7FFF] focus:ring-2 focus:ring-[#2B7FFF]/20 outline-none transition-all hover:border-gray-300"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-[#0B0F17] mb-2">Stage</label>
                      <select className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-[#2B7FFF] focus:ring-2 focus:ring-[#2B7FFF]/20 outline-none transition-all bg-white hover:border-gray-300">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[#0B0F17] mb-2">Message</label>
                    <textarea 
                      rows={4}
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-2">
                    ...
                    <button type="button" className="px-6 py-3 rounded-full font-medium text-[#0B0F17] border border-[#0B0F17]/20 hover:bg-[#0B0F17]/5 transition-all">
                      Partner With Us
                    </button>
                  </div>
                </form>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
              ...
              <div className="flex flex-wrap justify-center gap-8">
                <button onClick={() => scrollToSection(foundersRef)} className="text-white/60 hover:text-white transition-colors">For Founders</button>
                <button onClick={() => scrollToSection(investorsRef)} className="text-white/60 hover:text-white transition-colors">For Investors</button>
                <button onClick={() => scrollToSection(featuredRef)} className="text-white/60 hover:text-white transition-colors">Startups</button>
                <button onClick={() => scrollToSection(aboutRef)} className="text-white/60 hover:text-white transition-colors">About</button>
                <button onClick={() => scrollToSection(contactRef)} className="text-white/60 hover:text-white transition-colors">Contact</button>
              </div>
